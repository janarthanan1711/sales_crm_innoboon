...
class _CreateDealDialogState extends State<CreateDealDialog> {
  ...
  void initState() {
    ...
    if (isEdit) {
      ...
      _contactId = widget.deal!.contactIds.isNotEmpty
          ? widget.deal!.contactIds.first
          : null;
    }
    _loadOptions();
  }
  ...
  Future<void> _loadContacts() async {
    ...
    final res = await sl<GetContactsUseCase>()(
      GetContactsParams(accountId: id, limit: 100),
    );
    if (!mounted) return;
  ...
  Future<void> _onSave() async {
    ...
    result.fold(
      (failure) => ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to save deal: ${failure.message}'),
